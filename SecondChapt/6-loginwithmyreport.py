...
from SecondChapt.HTMLTestRunner_my import HTMLTestRunner
...
if __name__ == '__main__':
    ...
    #套件加载用例方法1：可以使用报告 （使用addTest）(只适用测试用例名字明确的情况）
    # mySuite = unittest.TestSuite()
    # mySuite.addTest(TestLoginClass("test"))

    #套件加载用例方法2：可以使用报告(makeSuite)（适用范围广）
    mySuite = unittest.TestSuite()
    mySuite.addTest(unittest.makeSuite(TestLoginClass))

    # #套件加载用例方法3：可以使用报告 (defaultTestLoader.loadTestsFromName)  ---   runner.run(discover)
    # mySuite = unittest.TestSuite()
    # mySuite.addTest(unittest.defaultTestLoader.loadTestsFromName("3-loginwithreport.TestLoginClass"))

    # #套件加载用例方法4：可以使用报告 (defaultTestLoader.loadTestsFromName)  ---   runner.run(discover)
    # # cash_path = os.path.join(os.getcwd(), '../SecondChapt')
    # ## print('文件地址：', os.getcwd())
    # ## print("cash_path:", cash_path)
    # mySuite = unittest.defaultTestLoader.discover(start_dir=r"D:\Users\Administrator\PycharmProjects\merchant\SecondChapt",pattern="loginddtgj.py")
    # #start_dir指定其实目录，pattern指定py文件
