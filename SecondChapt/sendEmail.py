...
class SendEmail(object):
    ...
    send_user = "xiangkaizheng@iapppay.com"  # 构建全局变量发送者
    email_host = "mail.iapppay.com"  # 构建全局变量邮件服务器的email_host（smpt）
    password = "iapppay002"  # 构建全局变量邮件服务器的password，邮箱服务器发送者的登录密码

    def send_mail(self,user_list,sub,content,filenamepath):   #收件人，主题，内容
         user = "xkz" +"<"+send_user+">"   #构建发送者
    ...
    def send_main_result_num(self,pass_count,fail_count,error_count,filenamepath,user_list,emailtitle):
        ...
        user_list = user_list
        sub = emailtitle
        content = "此次一共执行用例个数为%s个，成功个数为%s个，失败个数为%s个，错误个数为%s个，成功率为%s，失败率为的%s，错误率为%s." %(count_num,pass_num,fail_num,error_num,pass_result,fail_result,error_result)
...
if __name__ == '__main__':
    ...
    emailtitle = "web测试报告"
    sen.send_main_result_num(2,0,0,r'D:\Users\Administrator\PycharmProjects\merchant\SecondChapt\myreports\report.html',user_list,emailtitle)
    print("邮件已发送")